...
use image::{RgbImage, Rgb, EncodableLayout};
...
impl RayTracingPass {
    pub fn new(resolution: Resolution, sample_count: u32, max_bounces: u32) -> Self {
        RayTracingPass {
            ...
            sample_count,
            max_bounces,
            render_target: RgbImage::new(resolution.width(), resolution.height()),
        }
    }
...
impl Renderer {
    pub fn new(window: &mut glfw::Window, config: &RendererConfig) -> Self {
        ...
        let ray_tracing_pass = RayTracingPass::new(config.resolution, config.sample_count, config.max_bounces);
    ...
    pub fn render(&mut self, camera: &Camera, scene: &Scene) {
        self.ray_tracing_pass.execute(camera, scene);
        self.raster_pass.execute(&self.ray_tracing_pass.render_target);
    }

    pub fn save_render(&self, path: &Path) {
        self.ray_tracing_pass.render_target.save(path).expect("Failed to save output image");
    }
}
